...
def send_beon_whatsapp(phone_numbers: Union[str, List[str]], message: str) -> dict:
    """Send a WhatsApp message through the BeOn API.

    Returns a dictionary containing a success flag and any returned data/error details.
    """
    normalized_numbers = _build_phone_list(phone_numbers)
    if not normalized_numbers:
        logger.error("BeOn WhatsApp: phone number list is empty")
        return {"success": False, "error": "Phone number list is empty"}

    api_url = getattr(settings, "BEON_WHATSAPP_BASE_URL", "https://v3.api.beon.chat/api/v3/messages/whatsapp")
    token = getattr(settings, "BEON_SMS_TOKEN", None)  # Using same token as SMS

    if not token:
        logger.error("BeOn token is not configured")
        return {"success": False, "error": "BeOn token is not configured"}
    ...
    try:
        ...
        logger.info("BeOn WhatsApp sent to %s", normalized_numbers)
        return {"success": True, "data": data}
    except requests.RequestException as exc:
        logger.exception("Failed to send BeOn WhatsApp: %s", exc)
        error_detail = None
        if exc.response is not None:
            try:
                error_detail = exc.response.json()
